...
def read_files():
    try:
        ...
        if len(files) < 2:
            raise FileNotFoundError("Not enough files in the folder! Ensure at least two files are present.")
        
        # Read Excel files
        df_gps = pd.read_excel('GPS 2018-2023_NoContact.xlsx')
        df_speed = pd.read_excel('max_speed.xlsx')
...
def process_and_merge_dfs(df_gps, df_speed):
    try:
        ...
        print(f"KeyError: {key_error}. Check if the 'DATE' column exists in both DataFrames.")
        return None
...
if __name__ == "__main__":
    ...
    process_duplicates(df_gps, df_speed)
